...
from extrafuns import fun_name
# ------------------------------------------------------------
# ------------------------------------------------------------
    ...
    if len(pb) == 0:
        ...
        if len(artspubs) == 0:
            ...
            for k in range(len(ls_per_issn)):
                result = df_qualis[df_qualis['ISSN'] == ls_per_issn[k]].reset_index(drop=True)
                if len(result) == 0:
                    cc = 'XX'
                else:
                    cc = result.iloc[0, 2]
...
# ------------------------------------------------------------
# Eventos - Criado pela Helena
# ------------------------------------------------------------

def getevento(zipname):
    # lendo do zipfile
    #zipname = '3275865819287843.zip'
    zipfilepath = './xml_zip' + '/' + str(zipname)
    archive = zipfile.ZipFile(zipfilepath, 'r')
    ...
    if len(pb) == 0:
        ...
        artspubs = pb[0].find_all('trabalhos-em-eventos') # HELENA
        # VERIFICANDO se ha artigos em eventos # HELENA
        if len(artspubs) == 0:
            print('Artigos em eventos nao encontrados para', zipname) # HELENA
        else:
            # listas para armazenamento de dados EVENTOS # HELENA
            ls_trab_title = []
            ls_trab_year = []
            ls_trab_doi = []
            ls_trab_lang = []
            ...
            ls_trab_orders = []
            ls_jcr = []
            # A partir do grupo de artigos em eventos extrair os artigos # HELENA
            # em eventos
            artpub = artspubs[0].find_all('trabalho-em-eventos') # HELENA
            # a partir de cada artigo em evento extrair inf de interesse # HELENA
            for i in range(len(artpub)):
                # dados basicos do paper
                dba = artpub[i].find_all('dados-basicos-do-trabalho') # HELENA
                paperdb = str(dba)
                # definindo o nome do paper 
                result = re.search('titulo-do-trabalho=\"(.*)\" titulo-do-trabalho-i', 
                                   paperdb) # HELENA
                cc = fun_result(result)
                ls_trab_title.append(cc)
                # print(cc)
                # definindo ano do paper
                result = re.search('ano-do-trabalho=\"(.*)\" doi',
                                   paperdb) # HELENA
                cc = fun_result(result)
                ls_trab_year.append(cc)
                # print(cc)
                # definindo doi do paper
                result = re.search('doi=\"(.*)\" flag-divulgacao-c',
                                   paperdb)
                cc = fun_result(result)
                ls_trab_doi.append(cc)
                # print(cc)
                # definindo idioma do paper
                result = re.search('idioma=\"(.*)\" meio-de-divulgacao=',
                                   paperdb)
                cc = fun_result(result)
                ls_trab_lang.append(cc)
                # print(cc)
                # detalhamento do paper
                dda = artpub[i].find_all('detalhamento-do-trabalho') # HELENA
                paperdt = str(dda)
                # definindo nome do evento # HELENA
                result = re.search('nome-do-evento=\"([^\"]*)\"',
                                   paperdt) # HELENA
                cc = fun_result(result)
                ls_trab_evento.append(cc)
                # print(cc)
                # definindo isbn # HELENA
                ...
                for j in range(len(aut)):
                    ...
                    if fullname == nca:
                        ...
                        # print(fullname + ' ' + ncao)
                    # print(cc)
                ls_trab_authors.append(ls_allauthors)
                ls_trab_authororder.append(ls_allauthororder)
                ls_trab_orders.append(ls_authororder)
            # Qualis file # HELENA
            config_file = open('./config.txt', 'r')
            # config_file = open('./config.txt', 'r', encoding='utf-8')
            qf = config_file.readlines()[9].split(':')[1] # HELENA: Adicionei uma nova linha no config
            qf = qf.rstrip('\n')
            qf = qf.strip(' ')
            config_file.close()
            df_qualise = pd.read_csv(qf,
                                    header=0, sep='\t')
            # HELENA: Tenta casar o nome do evento com sigla ou nome no Qualis
            for k in range(len(ls_trab_evento)):
            	cc = 'XX'
            	result = df_qualise[df_qualise['NOME'].str.contains(ls_trab_evento[k],flags=re.I,regex=False)].reset_index(drop=True) # Casamento exato
            	if len(result) != 0:
                	cc = result.iloc[0, 2]		
				
				# Tenta alterar no nome do evento para ver se ele ocorre no Qualis
				# 1. Removendo menções à edição em números romanos                	
            	if cc == 'XX':
                	novo = re.sub(r"^[XVI]+ ",'',ls_trab_evento[k],flags = re.I)
                	result = df_qualise[df_qualise['NOME'].str.contains(novo,flags=re.IGNORECASE,regex=False)].reset_index(drop=True)
                	if len(result) != 0:
	                    cc = '{' + result.iloc[0, 2] + '}'
				# 2. Removendo menções à edição                	
            	if cc == 'XX':                	
                	novo = re.sub(r"^\d+(th)? ",'',ls_trab_evento[k],flags = re.I)
                	result = df_qualise[df_qualise['NOME'].str.contains(novo,flags=re.IGNORECASE,regex=False)].reset_index(drop=True)
                	if len(result) != 0:
	                    cc = '{' + result.iloc[0, 2] + '}'	                    
	                # A tratar: 12th LREC - 12th Language Resources and Evaluation Conference
				# 3. Removendo menções ao ano
            	if cc == 'XX':                	
                	novo = re.sub(r"\d\d\d\d",'',ls_trab_evento[k],flags = re.I)
                	result = df_qualise[df_qualise['NOME'].str.contains(novo,flags=re.IGNORECASE,regex=False)].reset_index(drop=True)
                	if len(result) != 0:
	                    cc = '{' + result.iloc[0, 2] + '}'	                    

				# Tenta encontrar a sigla do evento para ver se ela ocorre no Qualis
				# 1. Entre parenteses
            	if cc == 'XX':
                	busca = re.search("\(([A-Za-z]+)( \d\d\d\d)?\)",ls_trab_evento[k]) # Ex: (ICTAI) ou (DHandNLP 2020) ou (WebMedia)
                	if busca:
                		novo = busca.group(1).upper()                			                	
	                	#result = df_qualise[df_qualise['SIGLA'].str.contains(novo,flags=re.IGNORECASE,regex=False)].reset_index(drop=True)
	                	result = df_qualise[df_qualise['SIGLA'].str.upper() == novo].reset_index(drop=True)
	                	if len(result) != 0:
		                    cc = '[' + result.iloc[0, 2] + ']'	
		                #print("SIGLA = " + novo + " - " + cc)				
		        # 2. No início seguido de apóstrofe
            	if cc == 'XX':
                	busca = re.search("^([A-Za-z]+)[^\s]+\d+",ls_trab_evento[k]) # Ex: SBSI&apos;20: XVI Brazilian Symposium on Information Systems
                	if busca:
                		novo = busca.group(1).upper()
	                	result = df_qualise[df_qualise['SIGLA'].str.upper() == novo].reset_index(drop=True)
	                	if len(result) != 0:
		                    cc = '[' + result.iloc[0, 2] + ']'
		                #print("SIGLA = " + novo + " - " + cc)
		        # 3. No final do nome do evento            
            	if cc == 'XX':
                	busca = re.search("\- ([A-Za-z]+)( \d\d\d\d)?$",ls_trab_evento[k])
                	if busca:
                		novo = busca.group(1).upper()
	                	result = df_qualise[df_qualise['SIGLA'].str.upper() == novo].reset_index(drop=True)
	                	if len(result) != 0:
		                    cc = '[' + result.iloc[0, 2] + ']'
		                #print("SIGLA = " + novo + " - " + cc)
		        # 4. Só a sigla no nome do evento            
            	if cc == 'XX':
                	busca = re.search("^([A-Za-z]+)( \d\d\d\d)?$",ls_trab_evento[k]) # Ex: ICAART
                	if busca:
                		novo = busca.group(1).upper()
	                	result = df_qualise[df_qualise['SIGLA'].str.upper() == novo].reset_index(drop=True)
	                	if len(result) != 0:
            	...
            	cc = re.sub(r"[\{\}\[\]]",'',cc) # HELENA
            	ls_trab_qualis.append(cc)
                # print(cc)

            # DataFrame eventos # HELENA
            df_papers = pd.DataFrame({'TITLE': ls_trab_title,
                                      'YEAR': ls_trab_year,
                                      'DOI': ls_trab_doi,
                                      'LANG': ls_trab_lang,
                                      'EVENT': ls_trab_evento,
                                      'QUALIS': ls_trab_qualis,
                                      'ISBN': ls_trab_isbn,
                                      'AUTHOR': ls_trab_authors,
                                      'ORDER': ls_trab_authororder,
                                      'ORDER_OK': ls_trab_orders})
            latid = zipname.split('.')[0]
            pathfilename = str('./csv_producao/' + latid + '_event'  '.csv') # HELENA
            df_papers.to_csv(pathfilename, index=False)
            print(pathfilename, ' gravado com', len(
                df_papers['YEAR']), 'publicações em eventos') # HELENA
