...
#                print("\n\nTESTANDO INTERAÇÃO\n")
                authors = str(dd['AUTHOR'].iloc[o].upper()).split("', ")
#                print("Original: ", authors)
                for i in range(len(authors)):
                    authors[i] = authors[i].replace('[', '')
                    authors[i] = authors[i].replace(']', '')
                    authors[i] = authors[i].replace("'", '')
                    autor = authors[i].split(', ')
                    if len(autor) > 1:
                        authors[i] = autor[1] + ' ' + autor[0]   
#                print("Processado: ",authors)
...
#                             header=0)
    dfpaper = pd.read_csv('./csv_producao/eventos_all.csv',
                          header=0)
    dfpaper_uniq = pd.read_csv('./csv_producao/eventos_uniq.csv',
                               header=0)
    # paper uniq
    dfpaper['ID'] = dfpaper['ID'].apply(ss)
    dfpaper_uniq['ID'] = dfpaper_uniq['ID'].apply(ss)
...
#    dfppe_uniq_ext = dfppe_uniq_ext.sort_values(['YEAR_INI'])
    ...
    # verificando a interacao de eventos entre integrantes
    lsid = []
    lsid_tocompare = []
    lsinter_qtd = []
    for m in range(len(df_idlist)):
...
#                    print(authors[op])
    ...
    # data frame para profissionais sem interacao em eventos
    lsnointer_period = []
    for m in range(len(df_idlist)):
        aano = dfinterac[dfinterac['IDD'] == df_idlist.iloc[m, 0]]
        aasum = aano['WEIGHT'].sum()
        ...
        if aasum == 0 and aasum_a == 0:
            ...
            lsnointer_period.append(nointer)
    dfnointerac = pd.DataFrame({'NOME': lsnointer_period})
    dfnointerac.to_csv('./csv_producao/eventos_nointer.csv',
                       index=False, sep=',')
    # DANGER ATTENTION
    # dfinterac.to_csv('test.csv', index=False)
    # eliminando linhas sem interacao
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('./relatorio/figures/grapho_eventos.png')
    # plt.show()


# HELENA: Ainda precisa conferir se está fazendo certo
def getgraphoprojeto():
    # lendo a lista dos IDs e nome dos pesquisadores
    df_idlist = readIdList()
    # df_idlist['ID_LATTES'] = df_idlist['ID_LATTES'].apply(ss)
    # config_file = open('./config.txt', 'r')
    ...
    # verificando a interacao de projetos entre integrantes
    lsid = []
    lsid_tocompare = []
    lsinter_qtd = []
    for m in range(len(df_idlist)):
        ...
        for n in range(len(dfids_tocompare)):
            idd_tocompare = dfids_tocompare.iloc[n, 0]
            dd = dfppe_uniq[dfppe_uniq['ID'] == idd_tocompare]
            lsid.append(str(idd))
            lsid_tocompare.append(idd_tocompare)
            # DANGER ATTENTION FIX lname deve ser o nome completo
            # removendo caract desnecessarios
            interac = 0
            for o in range(len(dd)):
                authors = dd['INTEGRANTES'].iloc[o].upper()
    ...
    for m in range(len(df_idlist)):
        ...
        if aasum == 0 and aasum_a == 0:
            ...
            lsnointer_period.append(nointer)
    dfnointerac = pd.DataFrame({'NOME': lsnointer_period})
    dfnointerac.to_csv('./csv_producao/projetos_nointer.csv',
                       index=False, sep=',')
    # DANGER ATTENTION
    # dfinterac.to_csv('test.csv', index=False)
    # eliminando linhas sem interacao
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('./relatorio/figures/grapho_projetos.png')
    # plt.show()
