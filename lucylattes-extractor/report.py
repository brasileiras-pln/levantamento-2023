...
def getrelatorio():
    ...
    try:
        ...
        plt.title('Publicações em periódicos %i - %i' % (yyi, yyf))
...
#        acpq = acpq.filter(items=['QUALIS','TITLE','A1','A2','A3','A4','B1','B2','B3','B4','TOTAL'])
        # acpq
        plt.figure(figsize=(9, 5))
    ...
    # ------------------------------------------------------------
     # dfevento all
    try:
        dfevento = pd.read_csv('./csv_producao/eventos_all.csv',
                              header=0, dtype='str')
    except (OSError, IOError):
        print('------------------------------------------------------------\n' +
              'ATENCAO \n' +
              'Não há arquivo com evento all \n' +
              '------------------------------------------------------------')
        html_print_evento_all = 'NO'
    else:
        html_print_evento_all = 'YES'
        dfevento['YEAR'] = dfevento['YEAR'].replace('VAZIO', -99)
        pernum99 = dfevento[dfevento['YEAR'] == -99].reset_index(drop=True)
        if len(pernum99) >= 1:
            print('------------------------------------------------------------')
            print('ATENCAO: \n' + str(len(pernum99)) +
                  'artigos sem ano de publicacao')
            print('------------------------------------------------------------')
        # filtrando ano
        dfevento['YEAR'] = dfevento['YEAR'].apply(ff)
        dfevento = dfevento[(dfevento['YEAR'] >= yyi) & (dfevento['YEAR'] <= yyf)]
    # ------------------------------------------------------------    
    # dfevento unique - HELENA - Eventos
    try:
        dfevento_uniq = pd.read_csv('./csv_producao/eventos_uniq.csv', 
                                   header=0, dtype='str') # HELENA
    except (OSError, IOError):
        html_print_paper_uniq = 'NO'
        print('------------------------------------------------------------\n' +
              'ATENCAO \n' +
              'Não há arquivo com eventos unique \n' +
              '------------------------------------------------------------') # HELENA
    else:
        html_print_paper_uniq = 'YES'
        dfevento_uniq['YEAR'] = dfevento_uniq['YEAR'].replace('VAZIO', -99)
        pernum99 = dfevento_uniq[dfevento_uniq['YEAR']
                                == -99].reset_index(drop=True)
        # filtrando ano
        dfevento_uniq['YEAR'] = dfevento_uniq['YEAR'].apply(ff)
        dfevento_uniq = dfevento_uniq[(dfevento_uniq['YEAR']
                                     >= yyi) & (dfevento_uniq['YEAR'] <= yyf)]
        dfevento_uniq = dfevento_uniq.sort_values(['YEAR'])
        # GRAFICO artigo em evento evento # HELENA
        ace = dfevento_uniq.groupby(['YEAR'])['TITLE'].size().reset_index()
        ace_tot = ace['TITLE'].sum()
        plt.figure(figsize=(9, 5))
        plt.bar(x=ace['YEAR'], height=ace['TITLE'])
        plt.title('Publicações em eventos %i - %i' % (yyi, yyf))
        plt.xticks(np.arange(yyi, yyf + 1, 1))
        plt.xlabel('Ano')
        plt.ylabel('Número de publicações')
        plt.tight_layout()
        plt.savefig('./relatorio/figures/evento_dep_year.png')
        # plt.show()
        # HELENA - QUALIS PARA EVENTOS
        # GRAFICO artigo em evento por qualis
        
        # aceq
        #dfevento.filter(regex='[AB]',axis=1)
        aceq = dfevento.groupby(['QUALIS'])['TITLE'].size().reset_index()
        #aceq = aceq.rename(columns={'XX': 'SemQualisIdentificado'})
        plt.figure(figsize=(9, 5))
        plt.bar(x=aceq['QUALIS'], height=aceq['TITLE'])
        plt.title('Publicações em eventos %i - %i' % (yyi, yyf))
        plt.xlabel('Qualis')
        plt.ylabel('Número de publicações')
        plt.tight_layout()
        plt.savefig('./relatorio/figures/evento_year_qualis.png')
        # plt.show()
    ...
    htmlfile.write('<body> \n')
    # quick sumario
    htmlfile.write('<h0 class="title">Relatório de Produção gerado a partir das versões dos CVs Lattes públicas em 27/02/2023</h0> \n <br>\n')
    ...
    htmlfile.write(
        '<a href="https://doi.org/10.5281/zenodo.2591748"> <img src="https://zenodo.org/badge/DOI/10.5281/zenodo.2591748.svg" alt="DOI"> </a><br>')
    htmlfile.write('<small>Versão dos scripts usados para gerar este relatório: ')    
    htmlfile.write(
        '<a href="https://github.com/brasileiras-pln/levantamento-2023">https://github.com/brasileiras-pln/levantamento-2023</a></small>')
    ...
    htmlfile.write('<a href="#projetos">Projetos</a> \n <br>')
#    htmlfile.write('<a href="#projpesq">Projetos de pesquisa</a> \n <br>')
    htmlfile.write('<a href="#publicacoes">Publicações</a> \n <br>')
    htmlfile.write('<a href="#pubbookchap">Livros e capítulos</a> \n <br>')
    htmlfile.write('<a href="#pubperiod">Artigos em periódicos</a> \n <br>')
    htmlfile.write('<a href="#pubevento">Artigos em eventos</a> \n <br>') # HELENA
    htmlfile.write('<a href="#interacao">Interação entre pesquisadoras</a> \n <br>') # HELENA    
#    htmlfile.write(
...
#    htmlfile.write('<a href="#indcapes">Indicadores CAPES</a> \n <br>')
    ...
    dffullname.sort_values(by=['FULL_NAME'],ascending=True,inplace=True)
    ...
    # RESUMO
	#################################################            
    # resumo da produção projetos, periodicos, eventos, orientações
    htmlfile.write('<hr> \n')
    ...
    htmlfile.write('Artigos completos publicados em periódicos: ' +
                   (str(acp_tot) + '\n <br> \n'))
    htmlfile.write('Artigos em eventos: ' +
                   (str(ace_tot) + '\n <br> <br> \n'))
    ...
    if html_print_advi == 'NO':
        ...
        for oo in range(len(advi)):
            htmlfile.write('<li>' + str(advi.iloc[oo, 0]).lower().replace('-',' ').replace('_',' ').replace('cao','ção').replace('sao','são').replace('tifica','tífica') +
                           ': ' + str(advi.iloc[oo, 1]) + '\n</li>')
    htmlfile.write('\n <hr> \n \n')
    ...
    htmlfile.write('<a name="interacao"></a>' + '\n \n')
    htmlfile.write('<h1>Interação entre pesquisadoras do grupo '+ str(int(yyi)) + '-' + str(int(yyf)) + '</h1>\n')

    # Grafico de interacao no grupo em artigos de periódicos
    if html_print_paper_uniq == 'NO':
        htmlfile.write(
            'Não foi possivel gerar interação entre pesquisadoras em artigos de periódicos <br>\n')
    else:
        htmlfile.write('<figure> \n')
        htmlfile.write('<img src="./figures/grapho.png" alt="" ')
        htmlfile.write('width = "560" height = "auto" >\n')
        htmlfile.write(
            '<figcaption>Grafo de colaboração entre pesquisadoras em artigos de periódicos</figcaption>\n')
        htmlfile.write('</figure> \n')
        htmlfile.write('\n <br> \n \n')
        # Lista de profissionais sem interacao em periodicos - HELENA: optei por não imprimir
#        dfnointer_period = pd.read_csv('./csv_producao/periodicos_nointer.csv',
#                                       sep=',', header=0)
#        if len(dfnointer_period) > 0:
#            htmlfile.write(
#                '<b><i>Não foi possível localizar interações em periódicos para as seguintes pesquisadoras: </i></b><br> \n')
#            htmlfile.write('<ul>')
...
#        htmlfile.write('\n <hr> \n \n')
    ...
    # Grafico de interacao no grupo em artigos em eventos
    if html_print_paper_uniq == 'NO':
        htmlfile.write(
            'Não foi possivel gerar interação entre pesquisadoras em artigos em eventos <br>\n')
    else:
        htmlfile.write('<figure> \n')
        htmlfile.write('<img src="./figures/grapho_eventos.png" alt="" ')
        htmlfile.write('width = "560" height = "auto" >\n')
        htmlfile.write(
            '<figcaption>Grafo de colaboração entre pesquisadoras em artigos em eventos</figcaption>\n')
        htmlfile.write('</figure> \n')
        htmlfile.write('\n <br> \n \n')
        # Lista de profissionais sem interacao em eventos - HELENA: optei por não imprimir
#        dfnointer_evento = pd.read_csv('./csv_producao/eventos_nointer.csv',
#                                       sep=',', header=0)
#        if len(dfnointer_evento) > 0:
#            htmlfile.write(
#                '<b><i>Não foi possível localizar interações em eventos para as seguintes pesquisadoras: </i></b><br> \n')
#            htmlfile.write('<ul>')
#            for i in range(len(dfnointer_evento)):
#                name = dfnointer_evento.iloc[i, 0]
#                htmlfile.write('<b style="color:gray;"> <li>' +
#                               name + '</li> </b> \n')
#            htmlfile.write('\n </ul> \n <br> \n')
#        htmlfile.write('\n <hr> \n \n')

    # Grafico de interacao no grupo em projetos - Ainda precisa conferir se está fazendo certo
#    if html_print_paper_uniq == 'NO':
#        htmlfile.write(
#            'Não foi possivel gerar interação entre pesquisadoras em projetos <br>\n')
#    else:
#        htmlfile.write('<figure> \n')
#        htmlfile.write('<img src="./figures/grapho_projetos.png" alt="" ')
#        htmlfile.write('width = "560" height = "auto" >\n')
#        htmlfile.write(
#            '<figcaption>Grafo de colaboração entre pesquisadoras em projetos</figcaption>\n')
#        htmlfile.write('</figure> \n')
#        htmlfile.write('\n <br> \n \n')
    ...
    # PROJETOS
	#################################################            
    htmlfile.write('<h1>Projetos</h1> \n')
    htmlfile.write('<a name="projetos"></a>' + '\n \n')
    ...
    if html_print_ppe_uniq == 'NO':
        ...
        for idd in range(len(dfppe_uniq_ext)):
            ...
            ano = re.sub('\.0','',str(proj_yi)) # HELENA
            proj_yf = dfppe_uniq_ext.iloc[idd, 2]
            proj_au = dfppe_uniq_ext.iloc[idd, 4]
            htmlfile.write('<li>' + '\n')
            htmlfile.write('<i>' + '\n')
            htmlfile.write(str(proj) + '. início: ' +
                           str(ano) + '.' + ' fim: ' + str(proj_yf)
                           + '. ' + '<i><br>' + proj_au + '</i>.')
            htmlfile.write('\n </i>' + '\n')
            htmlfile.write('</li>' + '\n \n')
        htmlfile.write('</ol>')
    ...
    # PUBLICAÇÕES
	#################################################   
    htmlfile.write('<h1>Publicações</h1> \n')
    htmlfile.write('<a name="publicacoes"></a>' + '\n \n')
    ...
    if html_print_books_uniq == 'NO':
        ...
        ggt = (tabulate(gg, headers="keys", tablefmt='html',showindex=False,numalign="center"))
        # print(ggt)
        htmlfile.write(
    ...
    if html_print_paper_uniq == 'NO':
        ...
        htmlfile.write('width = "560" height = "auto" >\n')
        htmlfile.write(
            '<figcaption>Publicações de periódicos por Qualis.<br>XX: Não foi possível identificar automaticamente o Qualis</figcaption>\n')
        htmlfile.write('</figure> \n')
        htmlfile.write('\n <hr> \n \n')
    ...
    if html_print_paper_all == 'NO':
        ...
        gg = gg.groupby(['FULL_NAME', 'QUALIS'])[
            'TITLE'].size().unstack().reset_index(drop=False)
#        gg = gg.filter(items=['FULL_NAME','A1','A2','A3','A4','B1','B2','B3','B4','TOTAL'])
    ...
    # Tabela de periódicos nos quais o grupo publicou
    if html_print_paper_all == 'NO':
        htmlfile.write('Não há publicacao em periódico <br>\n')
    else:
        htmlfile.write(
            '<h2> Periódicos nos quais as integrantes do grupo publicaram ' +
            str(int(yyi)) + '-' + str(int(yyf)) + '</h2> \n <br> \n')

        gg = dfpaper_uniq
        
        gg['YEAR'] = gg['YEAR'].apply(lambda x: str(x).replace(".0",''))           	
        
		# Impressão de todos os Qualis juntos   
        gg = gg.groupby(['JOURNAL', 'YEAR'])['TITLE'].size().unstack().reset_index(drop=False)
        gg.fillna(0, inplace=True)
        gg['TOTAL'] = gg.sum(axis=1)
    ...
    htmlfile.write('<ol class="custom-counter">')
#    print("\n\n\nRelação de artigos em periódicos - INÍCIO\n") # Só para o artigo do WIT
    for idd in range(len(dfpaper_uniq)):
        pap = dfpaper_uniq.iloc[idd, 0]
        pap_yi = dfpaper_uniq.iloc[idd, 1]
        ano = re.sub('\.0','',str(pap_yi)) # HELENA
        pap_doi = dfpaper_uniq.iloc[idd, 2] # HELENA
        ...
        htmlfile.write('<li>' + '\n')
        htmlfile.write('<i>' + '\n')
        # HELENA - Alterei para imprimir o link para o artigo, se ele tiver DOI
        if str(pap_doi) != "":
        	htmlfile.write('<a href = \"https://doi.org/' + str(pap_doi) + '\">' + str(pap) + '</a>. ')        	
        else:
        	htmlfile.write(str(pap) + '. ')
#        print(pap) # Só para o artigo do WIT
        htmlfile.write(str(pap_jo) + ', <b>' + str(pap_qu) + '</b>, ' + 'ano: ' + ano)
        if pap_jc > 0:
          	htmlfile.write(', <b>' + 'JCR ' + str(pap_jc) + '</b>')
        htmlfile.write('<br>' + str(pap_au))
        htmlfile.write('\n </i>' + '\n')
        htmlfile.write('</li>' + '\n \n')
    htmlfile.write('</ol> \n')
#    print("\nRelação de artigos em periódicos - FIM\n\n\n")    # Só para o artigo do WIT

    # Publicacao em eventos - HELENA - INICIO
    htmlfile.write('<a name="pubevento"></a>' + '\n \n')
    htmlfile.write('<h1>Publicação em eventos</h1> \n')
    if html_print_paper_uniq == 'NO':
        htmlfile.write('Não há artigos em eventos <br>\n')
    else:
        # Grafico de producao de periodicos por ano
        htmlfile.write('<h2>Produção em eventos por ano</h2> \n')
        htmlfile.write('<figure> \n')
        htmlfile.write('<img src="./figures/evento_dep_year.png" alt="" ')
        htmlfile.write('width = "560" height = "auto" >\n')
        htmlfile.write(
            '<figcaption>Número de publicações por ano.</figcaption>\n')
        htmlfile.write('</figure> \n')
        htmlfile.write('\n <hr> \n \n')
        # Grafico de producao por qualis - HELENA
        htmlfile.write('<h2>Produção em eventos por Qualis</h2> \n')
        htmlfile.write('<figure> \n')
        htmlfile.write('<img src="./figures/evento_year_qualis.png" alt="" ')
        htmlfile.write('width = "560" height = "auto" >\n')
        htmlfile.write(
            '<figcaption>Publicações em eventos por Qualis.<br>XX: Não foi possível identificar automaticamente o Qualis</figcaption>\n')
        htmlfile.write('</figure> \n')
        htmlfile.write('\n <hr> \n \n')

    # ------------------------------------------------------------
    # Tabela de producao em eventos - HELENA
    if html_print_evento_all == 'NO':
        htmlfile.write('Não há publicacao em evento <br>\n')
    else:
        gg = dfevento
        gg = gg.groupby(['FULL_NAME', 'QUALIS'])[
            'TITLE'].size().unstack().reset_index(drop=False)
        ...
        # print(ggt)
        htmlfile.write(
            '<h2> Resumo da produção de artigos em eventos do grupo ' +
            str(int(yyi)) + '-' + str(int(yyf)) + '</h2> \n <br> \n')
        htmlfile.write(ggt + '\n <br> \n <br> \n')

    # Tabela de eventos nos quais o grupo publicou
    if html_print_paper_all == 'NO':
        htmlfile.write('Não há publicacao em evento <br>\n')
    else:
        htmlfile.write(
            '<h2> Eventos nos quais as integrantes do grupo publicaram ' +
            str(int(yyi)) + '-' + str(int(yyf)) + '</h2> \n <br> \n')
        ...
        # HELENA: Processa o nome do evento para remover no. edição, ano, etc
        # Remove menções à edição em números romanos    
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"^[XVI]+ ",'',str(x),flags = re.I))           	
		# Remove menções à edição                	
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d+th *",'',str(x),flags = re.I))
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d+st *",'',str(x),flags = re.I))
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d+nd *",'',str(x),flags = re.I))
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d+rd *",'',str(x),flags = re.I))
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d+º *",'',str(x),flags = re.I))
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d+o *",'',str(x),flags = re.I))
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d+ª *",'',str(x),flags = re.I))
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d+a *",'',str(x),flags = re.I))        
        # Remove menções ao ano
        gg['EVENT'] = gg['EVENT'].apply(lambda x: re.sub(r"\d\d\d\d",'',str(x),flags = re.I))     
        
		# Impressão de todos os Qualis juntos   
        gg = gg.groupby(['EVENT', 'YEAR'])['TITLE'].size().unstack().reset_index(drop=False)
        gg.fillna(0, inplace=True)
        gg['TOTAL'] = gg.sum(axis=1)
    ...
    # lista de publicacoes em eventos - HELENA
    htmlfile.write('<h2>Relação de artigos em eventos</h2> \n')
    htmlfile.write('<ol class="custom-counter">')
#    print("\n\n\nRelação de artigos em eventos - INÍCIO\n") # Só para o artigo do WIT
    for idd in range(len(dfevento_uniq)):
        pap = dfevento_uniq.iloc[idd, 0]
        pap_yi = dfevento_uniq.iloc[idd, 1]
        ano = re.sub('\.0','',str(pap_yi)) # HELENA
        pap_doi = dfevento_uniq.iloc[idd, 2] # HELENA
        pap_jo = dfevento_uniq.iloc[idd, 4]
        pap_qu = dfevento_uniq.iloc[idd, 5]        
        pap_au = dfevento_uniq['AUTHOR'].iloc[idd]
        htmlfile.write('<li>' + '\n')
        htmlfile.write('<i>' + '\n')
        if str(pap_doi) != "nan":
        	htmlfile.write('<a href = \"https://doi.org/' + str(pap_doi) + '\">' + str(pap) + '</a>. ')
 ...
 #       print(pap) # Só para o artigo do WIT	
        ...
        htmlfile.write('</li>' + '\n \n')
    htmlfile.write('</ol> \n')
#    print("\nRelação de artigos em eventos - FIM\n\n\n")    # Só para o artigo do WIT

	# HELENA - OPTEI POR NÃO IMPRIMIR
	
    # artigo completo periodico por qualis para cada pesquisador
...
#        htmlfile.write('\n <hr> \n')
    ...
    if html_print_advi == 'NO':
        ...
        advgg['NATURE'] = advgg['NATURE'].str.replace('INICIACAO CIENTIFICA', 'INICIAÇÃO CIENTÍFICA')
        advgg['NATURE'] = advgg['NATURE'].str.replace('Dissertação de mestrado', 'MESTRADO')
        advgg['NATURE'] = advgg['NATURE'].str.replace('Tese de doutorado', 'DOUTORADO')
        advgg['NATURE'] = advgg['NATURE'].str.replace('TRABALHO DE CONCLUSAO DECURSO GRADUACAO', 'TCC GRADUAÇÃO')
        advgg = advgg.groupby(['FULL_NAME', 'NATURE'])[
            'STUDENT'].size().unstack().reset_index(drop=False)
        # Optei por não imprimir orientações do tipo:
        # MONOGRAFIA DE CONCLUSAO DE CURSO APERFEICOAMENTO E ESPECIALIZACAO
        # ORIENTACAO DE OUTRA NATUREZA
        advgg = advgg.filter(items=['FULL_NAME','INICIAÇÃO CIENTÍFICA','MESTRADO','DOUTORADO','TCC GRADUAÇÃO','TOTAL'])
        advgg.fillna(0, inplace=True)
        advgg['TOTAL'] = advgg.sum(axis=1)
        advggt = (tabulate(advgg, headers="keys", tablefmt='html',showindex=False,numalign="center"))
        htmlfile.write(
...
#    htmlfile.write('<br> <br> <br>')
    ...
    htmlfile.write(
        'Relatório gerado por lucyLattes v1.1, <b>com algumas alterações realizadas por Helena Caseli (Brasileiras em PLN) em fevereiro/2023</b> Os resultados estão sujeitos a falhas devido a inconsistências no preenchimento dos CVs Lattes.')
    htmlfile.write('\n <br>')
    ...
    htmlfile.write('Para ter acesso à versão dos scripts usados para gerar este relatório, acesse: ')    
    htmlfile.write(
        '<a href="https://github.com/brasileiras-pln/levantamento-2023">https://github.com/brasileiras-pln/levantamento-2023</a> \n <br> \n')
    htmlfile.write('</footer> \n')
    htmlfile.close()
    # ------------------------------------------------------------
